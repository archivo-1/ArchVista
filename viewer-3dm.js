(function() {
    ...
    function updateModeUI() {
        ...
        const el = document.getElementById('mode-label'); // Changed from 'mode-label' if needed, but 'modchange' event is better
        ...
        document.querySelectorAll('.bar-btn').forEach(b => {
             if (b.dataset.mode) b.classList.toggle('active', b.dataset.mode === cameraMode);
        });
        ...
        for (let i = 0; i < layers.count; i++) {
            ...
            toggle.onclick = (e) => {
                ...
                modelGroup.traverse(obj => {
                    if (obj.userData && obj.userData.layerIndex === i) obj.visible = !isOff;
                });
            };
    ...
    function buildMatSet(hexColor, layerName) {
        ...
        const rendParams = Object.assign({
            side: side,
            color: hexColor,
            roughness: 0.75,
            metalness: 0.05,
            // FIX FLICKERING: logarithmicDepthBuffer is on, but we can also use polygonOffset
            polygonOffset: true,
        ...
        return {
            ...
            clay: new THREE.MeshStandardMaterial({ color: 0xd4c5b0, roughness: 0.8, metalness: 0.0, side: THREE.DoubleSide, polygonOffset: true, polygonOffsetFactor: 1, polygonOffsetUnits: 1 }),
    ...
    function loadModel() {
        ...
        rhino3dm().then(async function(rhino) {
            try {
                ...
                const near = Math.max(0.01, modelSpan * 0.00001); 
                const far = modelSpan * 50;
    ...
    function applyStyle(style) {
        ...
        document.querySelectorAll('.bar-btn').forEach(b => {
            if (b.dataset.style) b.classList.toggle('active', b.dataset.style === style);
        });
        if (!modelGroup) return;
        modelGroup.traverse(obj => {
    ...
    function getObjAppearance(obj, layerTable) {
        ...
        try {
            ...
            // Use object color if specified, otherwise layer color
            if (attrs.colorSource === 1 || attrs.colorSource === 'Object') {
                hexColor = rhinoColorToHex(attrs.objectColor);
    ...
    function init() {
        ...
        walkCamera = new THREE.PerspectiveCamera(75, aspect, 0.1, 10000);
        walkCamera.rotation.order = 'YXZ';

        flyCamera = new THREE.PerspectiveCamera(75, aspect, 0.1, 10000);
        ...
        (function anim() {
            ...
            if (cameraMode === 'walk' || cameraMode === 'fly') {
                ...
                if (move.lengthSq() > 0) {
                    ...
                    if (!checkCollision(cam.position, move.clone().normalize(), COLLISION_DISTANCE)) {
                        velocity.add(move);
                    }
                }
